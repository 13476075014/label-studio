    ...
    >
      <div className="w-full flex flex-col gap-2">
        <label className="w-full" htmlFor="project_name">
          {defaultT(t, "pages.create_project.project_name.name_title", "Project Name")}
        </label>
        <Input
          name="name"
          id="project_name"
      ...
      <div className="w-full flex flex-col gap-2">
        <label className="w-full" htmlFor="project_description">
          {defaultT(t, "pages.create_project.project_name.description_title", "Description")}
        </label>
        <TextArea
          name="description"
          id="project_description"
          placeholder={defaultT(t, "pages.create_project.project_name.description_placeholder", "Optional description of your project")}
          rows="4"
          style={{ minHeight: 100 }}
          value={description}
      ...
      {isFF(FF_LSDV_E_297) && (
        <div className="w-full flex flex-col gap-2">
          <label>
            {defaultT(t, "pages.create_project.project_name.Workspace", "Workspace")}
            <EnterpriseBadge className="ml-2" />
          </label>
          <Select placeholder={defaultT(t, "pages.settings.general.select_option", "Select an option")} disabled options={[]} triggerClassName="!flex-1" />
...
export const CreateProject = ({ onClose }) => {
  ...
  const steps = {
    name: <span className={tabClass.mod({ disabled: !!error })}>{defaultT(t, "pages.create_project.steps.name", "Project Name")}</span>,
    import: <span className={tabClass.mod({ disabled: uploadDisabled })}>{defaultT(t, "pages.create_project.steps.import", "Data Import")}</span>,
    config: defaultT(t, "pages.create_project.steps.config", "Labeling Setup"),
  };
  ...
  return (
    <Modal onHide={onDelete} closeOnClickOutside={false} allowToInterceptEscape fullscreen visible bare>
      <div className={rootClass}>
        <Modal.Header>
          <h1>{defaultT(t, "pages.create_project.title", "Create Project")}</h1>
          ...
          <Space>
            ...
            >
              {defaultT(t, "pages.create_project.steps.cancel", "Cancel")}
            </Button>
            <Button
              look="primary"
              size="small"
            ...
            >
              {defaultT(t, "pages.create_project.steps.save", "Save")}
            </Button>
          </Space>
        </Modal.Header>
        <ProjectName
