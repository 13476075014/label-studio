...
import type {
  ApiResponse,
  WrappedResponse,
} from "@humansignal/core/lib/api-proxy/types";
...
export const errorFormatter = (result: ApiResponse): FormattedError => {
  ...
  return {
    isShutdown,
    title: result.error
      ? defaultT(t, "common.runtime_error", "Runtime error")
      : defaultT(t, "common.server_error", "Server error"),
...
const handleGlobalErrorMessage = (
  result?: ApiResponse,
  errorFilter?: (result: ApiResponse) => boolean
) => {
  return (
    result?.error && (!isDefined(errorFilter) || errorFilter(result) === false)
  );
};

export const ApiProvider = forwardRef<ApiContextType, PropsWithChildren<any>>(
  ({ children }, ref) => {
    ...
    const callApi = useCallback(
      async <T,>(
        ...
        const shouldHandleGlobalErrorMessage = handleGlobalErrorMessage(
          result,
          errorFilter
        );
        ...
        if (
          ...
          "status" in result &&
          (result.status === 401 ||
            (IMPROVE_GLOBAL_ERROR_MESSAGES &&
              result.status === 404 &&
              shouldHandleGlobalErrorMessage))
        ) {
          apiLocked = true;
        ...
        if (result?.error) {
          ...
          let shouldShowGlobalError =
            shouldHandleGlobalErrorMessage && requestCompleted;
